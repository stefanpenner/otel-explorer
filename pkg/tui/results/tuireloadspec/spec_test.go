...
func TestInit(t *testing.T) {
	s := Init()
	if s.IsLoading != bool(false) {
		t.Errorf("IsLoading = %v, want %v", s.IsLoading, bool(false))
	}
	if s.ReloadGen != int64(0) {
		t.Errorf("ReloadGen = %v, want %v", s.ReloadGen, int64(0))
	}
	if s.FetchJob != int64(0) {
		t.Errorf("FetchJob = %v, want %v", s.FetchJob, int64(0))
	}
	if s.FetchGen != int64(0) {
		t.Errorf("FetchGen = %v, want %v", s.FetchGen, int64(0))
	}
	if s.StaleAccepted != bool(false) {
		t.Errorf("StaleAccepted = %v, want %v", s.StaleAccepted, bool(false))
	}
}
